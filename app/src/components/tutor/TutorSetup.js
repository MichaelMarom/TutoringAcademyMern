...
import { get_countries, get_gmt, get_rates, get_response, get_state, get_tutor_setup, get_tutor_setup_by_userId, post_tutor_setup, upload_form_one } from '../../axios/tutor';
...
const TutorSetup = () => {
    ...
    let saver = async () => {
        let response = await post_tutor_setup({ fname, uname, mname, sname, email: user[0].email, cell, acadId, add1, add2, city, state, zipCode, country, timeZone, response_zone, intro, motivation, headline, photo, video, tutorGrades, userId: user[0].SID })
                    ...
                    <div className="tutor-setup-top-field" >
                        ...
                        <div className="profile-details-cnt" style={{ float: 'left', margin: '0 10px 0 10px ', width: '40%' }}>
                            ...
                            <div style={{ display: 'flex', margin: '0 0 10px 0', padding: '0', alignItems: 'center', justifyContent: 'center', alignItems: 'center', width: '100%', whiteSpace: 'nowrap' }}>
                                ...
                                <input style={{ margin: '2.5px 0 0 0', width: '70%', float: 'right', position: 'relative' }} placeholder='Email' value={user[0].email} type="text" id="email" readonly />
                            </div>
